...
// Enable Eigen device functions for CUDA
...
// Device-compatible 2x2 determinant
__device__ inline float det2x2(float a, float b, float c, float d) {
  return a * d - b * c;
}
...
__device__ void calculateBarycentricCoordinate2DKernel(const Eigen::Matrix<float,3,2> &triangle, const Eigen::Vector2f &p, Eigen::Vector3f &w)
{
  ...
  // Manual determinant calculation for 2x2 matrices
  float det_numerator1 = det2x2(CA(0), CP(0), CA(1), CP(1));
  float det_denominator = det2x2(AB(0), AC(0), AB(1), AC(1));
  w(1) = det_numerator1 / det_denominator;

  float det_numerator2 = det2x2(AB(0), AP(0), AB(1), AP(1));
